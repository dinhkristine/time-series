...
Problem 1 from Lab\\
Part A\\
<<1a, echo=TRUE, eval=TRUE, background="white", include=TRUE, fig=TRUE, height=4, width=4>>=
set.seed(1)
...
<<1a2, echo=TRUE, eval=TRUE, background="white", include=TRUE, fig=TRUE, height=6, width=8>>=
set.seed(1)
bindat <- rbinom(n=10000, size=10, prob=0.8)
par(mfrow=c(2,2))
hist(bindat, breaks=seq(2, 10, 1), freq=F)
...
Part B\\
<<1b, echo=TRUE, eval=TRUE, background="white", include=TRUE>>=
1-pbinom(7, 10, 0.8)
@
